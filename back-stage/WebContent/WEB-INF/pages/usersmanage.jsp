...
<div class="admin-biaogelist">
    ...
    <form class="am-form am-g" action="usermana" method="get">
   		 <table id="bottom">
			<tr>
				...
				<td align="right" width="20%">请输入用户ID：</td>
				<td width="16%"><input type="text" id="seluser_id" name="seluser_id" class="am-form-field am-input-sm am-input-xm"   style="width:150px"/></td>
          ...
          <table width="100%" class="am-table am-table-bordered am-table-radius am-table-striped am-table-hover">
            <thead>
              <tr class="am-success">
                ...
                <th class="table-date am-hide-sm-only">用户性别</th>
            ...
            <tbody>
            	<c:forEach items="${pageInfo.list}" var="usermana" varStatus="status">
            	<tr class="goods1" id="${usermana.user_id}">
				<td><input type="checkbox" class="check" /></td>
				<td>${usermana.user_id}</td>
				<td>${usermana.nickname}</td>
				<td>${usermana.gender}</td>
				<td>${usermana.telephone}</td>
				<td>${usermana.email}</td>
				<td>
 	...
 	<div style="font-size:20px;">
		...
		<a href="usermana?pn=${pageInfo.navigateFirstPage}&recorddate=${requestScope.date}" style="margin-left:400px">首页</a>
		<c:if test="${pageInfo.hasPreviousPage}">
		<a href="usermana?pn=${pageInfo.prePage}&recorddate=${requestScope.date}">上一页</a>
		</c:if>
		<c:forEach items="${pageInfo.navigatepageNums}" var ="pageNum">
		<a href="usermana?pn=${pageNum}&recorddate=${requestScope.date}">${pageNum}</a>
		</c:forEach>
		<c:if test="${pageInfo.hasNextPage}">
		<a href="usermana?pn=${pageInfo.nextPage}&recorddate=${requestScope.date}">下一页</a>
		</c:if>
		<a href="usermana?pn=${pageInfo.navigateLastPage}&recorddate=${requestScope.date}">尾页</a>
		</span>
		</div>
