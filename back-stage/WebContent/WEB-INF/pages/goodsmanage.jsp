...
<div class="admin-biaogelist">
	...
	<form action="goodsmana1" method="post">
  ...
  <ul>
    ...
    <li><input type="text" class="am-form-field am-input-sm am-input-xm" name="selgoodss" style="width:100px" placeholder="商品搜索" value="${requestScope.msg}"/></li>
    <li><button type="submit" class="am-btn am-radius am-btn-xs am-btn-success" style="margin-top: -1px;">搜索</button></li>
  </ul>
</div>
</form>

    <form class="am-form am-g" action="goodsmanage2" method="post">
     		<table id="bottom">
			<tr>
          ...
          <table width="100%" class="am-table am-table-bordered am-table-radius am-table-striped">
            ...
            <tbody>
            	...
            	<tr class="goods1" id="${goods.goods_id}">
				<td><input type="checkbox" class="check" /></td>
				<td>${goods.goods_id}</td>
				<td>${goods.goods_name}</td>
				<td><c:forEach items="${goods.cate}" var="cate" varStatus="status">${cate.category_name} </c:forEach></td>
				<td>${goods.user_id}</td>
				<td>${goods.rel_price}</td>
				<td>
		...
		<a href="goodsmanage?pn=${pageInfo.navigateFirstPage}&recorddate=${requestScope.date}" style="margin-left:400px;color:black">首页</a>
		<c:if test="${pageInfo.hasPreviousPage}">
		<a href="goodsmanage?pn=${pageInfo.prePage}&recorddate=${requestScope.date}" style="color:black">上一页</a>
		</c:if>
		<c:forEach items="${pageInfo.navigatepageNums}" var ="pageNum">
		<a href="goodsmanage?pn=${pageNum}&recorddate=${requestScope.date}" style="color:black">${pageNum}</a>
		</c:forEach>
		<c:if test="${pageInfo.hasNextPage}">
		<a href="goodsmanage?pn=${pageInfo.nextPage}&recorddate=${requestScope.date}" style="color:black">下一页</a>
		</c:if>
		<a href="goodsmanage?pn=${pageInfo.navigateLastPage}&recorddate=${requestScope.date}" style="color:black">尾页</a>
		</span>
		</div>
